...
${MyHostname}    demo6733
${MyRepositoryName}    TMUPLOADERV497TC4165
...
Test17
    ...
    ${MyValue} =    Get Value    id=gwt-uid-3
...
Test19
    [Documentation]    Click the button "Reset" to start filling the form again
    [Tags]    CRITICALITY HIGH
    Wait Until Element Is Visible    id=resetButton    timeout=15s
    Set Focus To Element    id=resetButton
...
Test20
    ...
    Wait Until Page Contains    Are you sure you want to reset form (clear uploaded files and entered data)    timeout=15s
    Wait Until Element Is Visible    id=rightButton    timeout=15s
    Take My Screenshot
    Click Element    id=rightButton
...
Test21
    [Documentation]    The form has been reset in the main interface, the user can fill in the fields again
    [Tags]    CRITICALITY NORMAL
    Wait Until Page Contains    At least the patient name field and one recipient must be filled    timeout=15s
    Wait Until Element Is Visible    id=patientFamilyNameField    timeout=15s
    Wait Until Element Is Visible    id=patientGivenNameField    timeout=15s
    ...
    ${MyValue} =    Get Value    id=gwt-uid-3
    Should Contain    ${MyValue}    Patient Family Name
